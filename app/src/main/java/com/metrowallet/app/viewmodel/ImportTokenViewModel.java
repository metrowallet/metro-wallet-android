...
import com.metrowallet.app.entity.tokens.Ticket;
...
import static com.metrowallet.app.entity.MagicLinkParcel.generateReverseTradeData;
import static com.metrowallet.token.tools.ParseMagicLink.currencyLink;
import static com.metrowallet.token.tools.ParseMagicLink.customizable;
import static com.metrowallet.token.tools.ParseMagicLink.normal;
import static com.metrowallet.token.tools.ParseMagicLink.spawnable;
import static com.metrowallet.token.tools.ParseMagicLink.unassigned;
...
public class ImportTokenViewModel extends BaseViewModel
{
    ...
    public void importThroughFeemaster(String url)
    {
        try
        {
            initParser();
            MagicLinkData order = parser.parseUniversalLink(universalImportLink);
            disposable = metroWalletService.handleFeemasterImport(url, wallet.getValue(), network.getValue().chainId, order)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
    ...
    {
        disposable = metroWalletService.checkFeemasterService(feemasterServer, network.getValue().chainId, importOrder.contractAddress)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
